...
impl From<NeedExample> for Need {
    fn from(need_example: NeedExample) -> Self {
        ...
        Self {
            current: need_example.current + rng.random_range(-variance..variance),
            max: need_example.max + rng.random_range(-variance..variance),
            rate: need_example.rate + rng.random_range(-variance..variance),
            low: need_example.low + rng.random_range(-variance..variance),
            normal: need_example.normal + rng.random_range(-variance..variance),
            high: need_example.high + rng.random_range(-variance..variance),
        }
    }
}
...
impl UnitTemplate {
        ...
        Self {
            ...
            component_builders: vec![
                |commands: &mut Commands, entity: Entity| {
                    commands.entity(entity).insert(HasName { name: ["Crab", "Carl", "Rusty"][rand::rng().random_range(0..3)].to_string() });
                },
            ],
        }
    }
    ...
    pub fn random_afflictions_humanoid() -> Vec<Affliction> {
        let random_affliction_pair = || {
            ...
            let mut rng = rand::rng();
            let i1 = rng.random_range(0..affliction_types.len());
            let i2 = rng.random_range(0..affliction_locations.len());
            (affliction_types[i1], affliction_locations[i2])
        };
        let a = random_affliction_pair();
        let b = random_affliction_pair();
        let random_affliction_pair_internal = || {
            let affliction_locations = vec![
        ...
        let c = random_affliction_pair_internal();
        let d = random_affliction_pair_internal();
        ...
        let num_afflictions = rng.random_range(0..=2);
    ...
    pub fn random_skillset_humanoid() -> Skillset {
        ...
        let mut loss_values: Vec<i32> = losses.iter().map(|range| rng.random_range(range.clone())).collect();
